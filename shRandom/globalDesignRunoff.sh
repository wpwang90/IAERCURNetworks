...
#   from $YSTART/Jan/01 00:00 to $YEND/Dec/31 24:00
#   - simulation is done in 1-year loop
#   - daily input runoff & daily output
# =========================================================

# CaMa-Flood base directory
# CAMADIR="/Users/yamadai/work/CaMa-Flood"
CAMADIR="/home/woqie/Documents/CaMa-Flood_v3.6.2_20140909"

cd ${CAMADIR}/gosh
RunoffName='Guangxi';
NameList='ChinaPro';
RandomTypeName=("PearsonIII","Random");
RandomPrintName=("PearsonIII","Uniform");
runoffNameList=("Henan","Hunan","Sichuan","Zhejiang");
totalRunoff={#runoffNameList[*]}-1
totalRandom={#runoffNameList[*]}-1
tempName="ABCDEFGHIJKLMNOPQRSTUVWXYZ";

for ktotalRandom in $( seq 0 $totalRandom )
do
for ktotalRunoff in $( seq 0 $totalRunoff )
do
for ktempName in $( seq 0 19 )
do
...
EXP="global_15min"                    #   experiment name (output directory name)
# EXP="region_15min"                    # regional simulation
...
##### Map & Topography ################
FMAP="${BASE}/map/global_15min"       #   map directory
# FMAP="${BASE}/map/region_15min"       # (regional map) 
...
CROFDIR="/home/woqie/Documents/designData/"$NameList/${runoffNameList[$ktotalRunoff]}${RandomTypeName[$ktotalRandom]}/;    #   runoff directory
CROFPRE="wRoff"${runoffNameList[$ktotalRunoff]}${RandomPrintName[$ktotalRandom]}${tempName[$ktempName]}                     #   runoff prefix/suffix  ( $(PREFIX)yyyymmdd$(SUFFIX) )
CROFSUF=".one"
...
mkdir="/home/woqie/Documents/designOuttemp/"${runoffNameList[$ktotalRunoff]}${RandomTypeName[$ktotalRandom]}/${tempName[$ktempName]}/
COUTDIR="/home/woqie/Documents/designOuttemp/"${runoffNameList[$ktotalRunoff]}${RandomTypeName[$ktotalRandom]}/${tempName[$ktempName]}/ # output directory 
