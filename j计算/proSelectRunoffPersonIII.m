...
nt=365;
pearsonIIIRandomNumbers=xlsread('C:\Users\woqie\Mac文档\研究\BU\河流公路铁路耦合系统\程序\j计算\PersonIIIRandomNumbers.xls')
RandomNumbers=pearsonIIIRandomNumbers(:,2)/max(pearsonIIIRandomNumbers(:,2))*20;
%RunoffName='Guangxi';
%NameList='ChinaPro';
NameList='USAState';
...
temp=['A':'Z'];
outfilepath= ['D:\\径流数据设定\\',NameList,'\\',RunoffName,RandomTypeName,'\\','wRoff',RunoffName,RandomTypeName,temp(totalIterRoff)];
...
totalI={};
 for it=1:length(setValue)
    ...
    for i=1:length(xx)
     temp(xx(i),yy(i))=setValue(it)-RandomNumbers(randi(length(RandomNumbers)));
    end
    totalI{it}=temp;
    nx=360;
    ny=180;
