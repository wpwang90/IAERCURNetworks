...
ylimmin=0.99999*min(c(Res2,afterAttackRandomTotal1/iTersum,afterAttackRandomShellTotal1/iTersum));
ylimmax=1.00001*max(c(Res2,afterAttackRandomTotal1/iTersum,afterAttackRandomShellTotal1/iTersum));


Res=data.frame(Res1,Res2,"Blue");
names(Res)=c("x","y","AType")
AttackRandom=data.frame(afterAttackRandomSum,afterAttackRandomTotal1/iTersum,"Red");
names(AttackRandom)=c("x","y","AType")
AttackShell=data.frame(afterAttackRandomSum,afterAttackRandomShellTotal1/iTersum,"Green");
names(AttackShell)=c("x","y","AType")

finalRes=rbind(Res,AttackRandom,AttackShell);
...
tempP=tempP+ geom_point(data=Res,aes(x=x,y=y),shape=19, alpha=0.5,size=4,color="Blue")
tempP=tempP+theme_bw()+theme(panel.grid=element_blank())
tempP=tempP+ggtitle(paste0(runoffName,"RoadUnderAttacks"))
print(tempP)
ggsave(file=paste0(".\\结果\\",paste0(runoffName,"RoadUnderAttacks.pdf")))
ggsave(file=paste0(".\\结果\\",paste0(runoffName,"RoadUnderAttacks.tiff")))
